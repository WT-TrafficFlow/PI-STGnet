...
class ST_Block():
    ...
    def spatiotemporal(self, bn, bn_decay, is_training, speed=None, STE=None, supports=None,mask=False, speed_all=None, adj =None):
        X = speed
        X_ALL=speed_all
        # gcn 静态空间
        XS = tf.transpose(X + X_ALL[:, self.input_length:] + STE, perm=[0, 2, 1, 3])
        XS = tf.reshape(XS, shape=[-1, self.site_num, self.emb_size * self.input_length])
        gcn = self.model_func(self.placeholders,
                              input_dim=self.emb_size * self.input_length,
                              para=self.para,
                              supports=supports)
        XS = gcn.predict(XS)

        XS = tf.reshape(XS, shape=[-1, self.site_num, self.input_length, self.emb_size])
        XS = tf.transpose(XS, perm=[0, 2, 1, 3])
        ...
        XSL = gatedFusion(XS, XL, self.emb_size, bn, bn_decay, is_training)
        # XSL = tf.add(X, XSL)

        # XSL = tf.add(XS, XL) # 消融静态融合门
        # XSL = XS # 消融静态时空
        # GATs
        ...
        for _ in range(self.para.num_blocks):
            HT = temporalAttention(HT, STE, self.num_heads, self.emb_size // self.num_heads, bn, bn_decay, is_training, mask=mask)
        ...
        for _ in range(self.para.num_blocks):
            HS = spatialAttention(HS, STE, self.num_heads, self.emb_size // self.num_heads, bn, bn_decay, is_training)


        H = gatedFusion(HS, HT, self.emb_size, bn, bn_decay, is_training)
        # H = tf.add(HS, HT) #消融att的融合门

        X = tf.add(X, H)
        # X = XSL #其他不动 加这一行 att就都没了
...
def gatedFusion(HS, HT, D, bn, bn_decay, is_training):
    ...
    z = tf.nn.sigmoid(tf.add(XS, XT))
    # XST = tf.add(HS, HT)
    # XST = tf.concat([HS, HT], axis=-1)  # 这个也试一下
    
    # z = FC(
    #     tf.concat([HT, HS], axis=-1), units=D, activations=tf.nn.sigmoid,
    #     bn=bn, bn_decay=bn_decay,
    #     is_training=is_training, use_bias=True)
    XST = FC(
        tf.concat([HT, HS], axis=-1), units=D, activations=None,
        bn=bn, bn_decay=bn_decay,
        is_training=is_training, use_bias=True)
    H = tf.multiply(z, XST)
    ...
    # H = FC(
    #     H, units=[D, D], activations=[tf.nn.relu, None],
    #     bn=bn, bn_decay=bn_decay, is_training=is_training)  # 留这个和不留这个都尝试一下
    # z = tf.nn.sigmoid(tf.add(XS, XT))
    # H = tf.add(tf.multiply(z, HS), tf.multiply(1 - z, HT))
    H = FC(
        H, units=[D, D], activations=[tf.nn.relu, None],
